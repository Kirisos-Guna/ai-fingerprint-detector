...
- `AIFingerprintDetector.exe` (~63 MB, with app icon and Windows version metadata)
- `chromium/` (portable Chromium used automatically by the exe)

The exe carries an embedded icon (shown in Explorer and the taskbar) and
version metadata (Product/File version `1.0.0`, description, copyright) —
edit `detector/version.py` to bump it. `assets/icon.ico` is generated by
`make_icon.py`.
